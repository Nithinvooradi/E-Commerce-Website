  ...
  reducers: {
    ...
    filterGender: (state, action) => {
      try {
        ...
        if (oneGenderType) {
          state.error = false;
          const saveItem = JSON.stringify(gender);
          sessionStorage.setItem("filteredData", saveItem);
        } else {
          state.error = true;
    ...
    sortByPrice: (state, action) => {
      try {
        ...
        if (count > 1) {
          ...
          if (!noError) {
            state.filteredProducts = price;
            const saveItem = JSON.stringify(price);
            sessionStorage.setItem("filterdedData", saveItem);
          }
        } else {
          state.error = true;
          state.filteredProducts = [];
    ...
    filterByColor: (state, action) => {
      try {
        ...
        if (color.length <= 0) {
          ...
          state.error = false;
          state.filteredProducts = color;
          const saveItem = JSON.stringify(color);
          sessionStorage.setItem("filteredData", saveItem);
        }
      } catch (err) {
        return err;
    ...
    filterBySize: (state, action) => {
      try {
        ...
        if (size.length <= 0) {
          ...
          state.error = false;
          state.filteredProducts = size;
          const saveItem = JSON.stringify(size);
          sessionStorage.setItem("filteredData", saveItem);
        }
      } catch (err) {
        return err;
